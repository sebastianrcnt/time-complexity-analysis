...
public class BinarySearch {
  ...
  /**
   * Searches an element {@param n} in a sorted array {@param a} and returns its index in O(log n)
   * time. The Index may not correspond to the first occurrence of the element.
   *
  ...
  public static int binarySearch(int[] a, int n, int low, int high) {
    ...
    if (low <= high) {
      int mid =
          (low + high) / 2; // to prevent overflow you can instead do: mid = low + (high - low) / 2
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        binarySearch(
            new int[] {0, 1, 2, 2, 2, 3, 3}, 2)); // doesn't return index of first occurrence
    System.out.println("---------");
